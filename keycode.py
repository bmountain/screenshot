...
class NumpadmadnessWindow(tkinter.Tk):
    def __init__(self):
        ...
        l = tkinter.Label(
            self,
            text="Click here and type keys on your Numpad.\nObserve event codes on standard output.",
        )
    ...
    def keyrelease(self, event):
        print(
            time.time(),
            "KEYRELEASE {char!r} keysym='{keysym}' keycode={keycode} keysym_num={keysym_num} state={state}".format(
                **vars(event)
            ),
        )  # XXX
        if sys.platform == "darwin":
            ...
            if event.keysym_num in self.joystick_keys_windows_keysym_num:
                print(
                    "JOYSTICK switch OFF:",
                    self.joystick_keys_windows_keysym_num[event.keysym_num],
                )
                return
            elif event.keycode in self.joystick_keys_windows_keycode:
                print(
                    "JOYSTICK switch OFF:",
                    self.joystick_keys_windows_keycode[event.keycode],
                )
                return
    ...
    def keypress(self, event):
        print(
            time.time(),
            "KEYPRESS {char!r} keysym='{keysym}' keycode={keycode} keysym_num={keysym_num} state={state}".format(
                **vars(event)
            ),
        )  # XXX
        if sys.platform == "darwin":
